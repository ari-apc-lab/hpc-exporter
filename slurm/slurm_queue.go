...
func (sc *SlurmCollector) collectQueue() {
	...
	for fields, err := nextLine(); err == nil; fields, err = nextLine() {
		...
		if state == "idle" || state == "allocated" || state == "mixed" {
			...
			// Size
			// Parse range
			_, mapContainsKeyPartitionSizeLower := metrics["PartitionSizeLower"][partition]
			if !mapContainsKeyPartitionSizeLower {
				metrics["PartitionSizeLower"][partition] = []float64{}
			}
			_, mapContainsKeyPartitionSizeUpper := metrics["PartitionSizeUpper"][partition]
			if !mapContainsKeyPartitionSizeUpper {
				metrics["PartitionSizeUpper"][partition] = []float64{}
			}
			...
			metrics["PartitionSizeLower"][partition] = append(metrics["PartitionSizeLower"][partition], sizeLower)
			metrics["PartitionSizeUpper"][partition] = append(metrics["PartitionSizeUpper"][partition], sizeUpper)

			// Time
			// Parse time (day-hours:minute:second)
			metrics["PartitionTime"][partition] = append(metrics["PartitionTime"][partition], parseTime(fields[qsiTIME]))
		}
		collected++
	}
	...
	averageMetrics := []string{
		...
		"PartitionMemory", "PartitionSizeLower", "PartitionSizeUpper",
		"PartitionTime",
	}
	...
	log.Infof("%d queued partitions collected", collected)
}
